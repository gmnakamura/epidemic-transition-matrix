...
contains
  ...
  !----------------------------------------------------------------------
  !----------------------------------------------------------------------
  subroutine create_random_adjacency(N,p,B,A)
    !objective:
  ...
  !----------------------------------------------------------------------
  !----------------------------------------------------------------------
  subroutine create_random_adjacency_ensemble(N,M,p,B)
    !objective:
    !          evaluate the ensemble average B of random adj. matrices A
    !procedure:
    !          call create_random_adjacency and add to the average
    !variables:
    !          N = graph size
    !          M = ensemble size
    !          p = connection probability
    !          B = average graph
    integer::N,M
    real*8 ::p,B(0:N-1,0:N-1),C(0:N-1,0:N-1)
    integer::A(0:N-1,0:N-1)
    integer::irun
    real*8 ::rinv
    B=0d0
    ...
    do irun=1,M
       ...
       call create_random_adjacency(N,p,C,A)
       B=B+(A*rinv)
    end do
  end subroutine create_random_adjacency_ensemble
  !----------------------------------------------------------------------
  !----------------------------------------------------------------------
  subroutine create_time_generator(N,imax_size,params,A,H)
    ...
    !          N        = size
    !          imax_size= 2**N -1
    !          A        = (average) adjacency matrix ( i=0..N-1, j=0..N-1)
